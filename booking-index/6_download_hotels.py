    ...
    for i, url in enumerate(urls):
        ...
        if url:
            try:
                ...
                file_name = str(i).zfill(8) + '-' + url.split("/")[-1]
                
                # Save the content to the file
                with open(os.path.join(download_folder, file_name), 'wb') as output_file:
                    output_file.write(response.content)
                
                print(f"Downloaded: {url} - {i/len(urls)*100}% - {i}/{len(urls)}")
            except requests.exceptions.RequestException as e:
                print(f"Failed to download {url}: {e}")

if __name__ == "__main__":
    file_path = '5@_italian_hotels_list.txt'  # Path to the file containing the URLs
    download_folder = '6@_italian_hotels'  # Folder to save the downloaded files
